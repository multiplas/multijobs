...
function workscout_job_listing_category_edit_meta_field($term) {
	...
	$term_meta = get_option( "taxonomy_$t_id" ); 
	 ?>
	<tr class="form-field">
		<th scope="row" valign="top"><label for="term_meta[fa_icon]"><?php esc_html_e( 'Category Icon', 'workscout' ); ?></label></th>
		<td>
				...
				if ( ! is_wp_error( $terms ) ) {
					if ( $map_link ) {
						foreach( $terms as $term ) {
							echo '<a href="' . get_term_link( $term->slug, 'job_listing_region' ) . '">' . esc_html( $term->name ) . '</a> '; 
						}
					} else {
						foreach( $terms as $term ) {
